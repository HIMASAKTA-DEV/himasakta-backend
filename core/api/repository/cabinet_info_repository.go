...
func (r *cabinetInfoRepository) GetCurrentCabinet(ctx context.Context, tx *gorm.DB) (entity.CabinetInfo, error) {
	...
	if err := tx.WithContext(ctx).Model(&entity.CabinetInfo{}).Preload("Logo").Preload("Organigram").Preload("Feeds").Where("is_active = ?", true).Order("period_start desc").First(&ci).Error; err != nil {
...
func (r *cabinetInfoRepository) GetById(ctx context.Context, tx *gorm.DB, id uuid.UUID) (entity.CabinetInfo, error) {
	...
	var ci entity.CabinetInfo
	if err := tx.WithContext(ctx).Preload("Logo").Preload("Organigram").Preload("Feeds").Take(&ci, "id = ?", id).Error; err != nil {
		return entity.CabinetInfo{}, err
	}
	return ci, nil
}
